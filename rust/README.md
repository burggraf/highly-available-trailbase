...
## Local authenticated transport (Task 9G through Task 9M accepted locally)

Task 9G frames the validated node command with a bounded length prefix and strict versioned JSON envelope. A printable per-channel token is checked with bounded constant-time comparison; wrong tokens, malformed frames, duplicate/unknown fields, oversized input, and invalid nested commands refuse. Task 9H exercises one temporary Unix listener request in a private mode-0700 parent, reads one declared-length frame without requiring peer half-close, and removes only the verified bound path. Task 9I wraps this in a library/test-only persistent local bind/receive/shutdown/rebind lifecycle. Task 9J connects valid transport only to a supplied injected executor, preserving refusal and uncertainty. Task 9K adds strict bounded outcome/refusal response frames. Task 9L adds a one-request close-synchronized requester with no retry/fallback. Task 9M adds a library/test-only action-adapter client that conservatively maps requester uncertainty; no production listener, TLS/mTLS, or remote/native action is configured.
