...
Tasks 2 through 8 and Tasks 9C through 9M are accepted on `main` for their stated local boundaries. Task 9D adds only a typed, injected local action-adapter contract: the normal server still uses an unavailable adapter and refuses native actions. Task 9E adds only strict in-memory node command/observation envelopes; Task 9F validates their in-memory forwarding boundary with an injected executor. Task 9G is limited to token-authenticated local framing; Task 9H exercises one temporary Unix listener request; Task 9I exercises persistent local bind/receive/shutdown/rebind lifecycle; Task 9J connects that transport only to an injected executor; Task 9K adds bounded response framing; Task 9L adds a one-request close-synchronized local requester; Task 9M adds a library/test-only action-adapter client with conservative uncertainty mapping. TLS/mTLS, remote forwarding, and native execution remain separately qualified. The Task 9C console is loopback/SSH-tunnel friendly, derives the sole mutation authority from `controller_node` plus local `--node-id`, and shows unknown observations. One bounded, cleaned-up disposable probe used fresh test roots on fm1/fm2 and the checked-out Rust proxy; it did not qualify deployment, fencing, public HTTPS, or native failover/rejoin. See `docs/reports/v1-task9b-*` and `docs/reports/v1-task9c-*`.
