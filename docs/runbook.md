...
- **Blockers:** checked-in source/tests now require exact completed-route writer/epoch/key-set/config-hash identity and the exact incomplete-D2 route-pending prefix, but this has not been deployed or live-validated and does not qualify live route authority. No objective unified restore acceptance manifest/schema enumerates required checks and source/target/epoch/position/evidence bindings; this checklist is a stop boundary, not authorization to conduct or accept a new restore.
