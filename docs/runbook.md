...
At source checkpoint `ce1b66651a5c442f44e6459fe17ada9f4ab441b6`, the controller-built `hat-restore-acceptance-1` request/result contract is closed in source and tests. The journal migration is exact and fail-closed: new rows use the contract; completed legacy rows retain only their named authority/reconciliation compatibility; unfinished legacy rows cannot continue, activate, route, reconcile, or finish. The controller constructs the canonical request and validates the canonical oracle result, including descriptor-bound retention and durable ordering. Tests cover journal migration, request/result boundaries, descriptor identity/durability, crash/reopen and replay refusal. No manifest is route authority.

Final verification passed 181 focused tests with 235 subtests, 235 runtime tests, 115 D4 tests, `py_compile`, and `git diff --check`; runtime discovery retained non-failing SQLite `ResourceWarning`s. The final safety review's oracle service-account ownership blocker and its failing-first test remain historical evidence; `ce1b666` aligns private ledger/output ownership with `hat-oracle` while preserving controller-held source authority. The prior `446097f` verification with two stale fsync-order expectation failures and `08bbe86` verification with one stale cleanup-uncertainty expectation remain historical failure evidence and were not erased by the corrected run. Intermediate combined runs also preserved order-dependent subprocess-import failures and stale mock/fixture expectations while descriptor, operation-evidence, and cleanup boundaries were tightened; later passing runs do not erase them. Malformed `cd` and cleanup commands remain tooling errors, not test evidence.

This is source/test evidence only. It is not deployment, live qualification, fresh historical validation, zero-loss HA, or production readiness. Preserve the limitations below: D4 is infeasible on stock TrailBase v0.33.11; ACK/recoverability/provider/distributed-authority, controller SPOF, capacity/fault-domain, partition, certificate, and load constraints remain incomplete.
...
- **Restore verification (not non-disruptive):**
  ...
  - **Acceptance:** accept restore verification only when the canonical `hat-restore-acceptance-1` result validates against its exact request and operation, including required integrity, authentication/session, record/membership, fault, and exact restore-position/proof checks. Any missing, mismatched, legacy, or uncertain evidence refuses acceptance and escalates; a rerun is not a substitute. This is repository contract closure, not live/deployment qualification.
...
- **Blockers:** checked-in source/tests now close the exact restore acceptance, D2/D3 consumer, descriptor identity/ownership, and CLI position-evidence contracts. These repository contracts have not been deployed or live-validated and do not qualify live route authority, zero-loss behavior, or production readiness; remaining live/deployment qualification is explicitly outstanding.
