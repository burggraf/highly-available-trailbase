...
Ingress startup executes `hat ingress-check`: this is a read-only admission check and does not itself change service state. Its four incomplete/completed branches are: (1) completed operation, requiring maintenance absent and completed route evidence matching the current ingress config hash; (2) incomplete verified-D3 rejoin-tail, requiring the maintenance marker, valid D3 proof, retained route writer/epoch, and matching route hash, without permit/boot/PID; (3) incomplete D3 `route_pending`/`verify_pending`, requiring the exact D3 phase prefix, maintenance/no failure, D3 proof as applicable, and permit operation/current boot/PID/birth/config hash, with `verify_pending` additionally validating recorded route writer/epoch/config facts; and (4) incomplete D2 `route_pending`, requiring maintenance operation identity, no failure, `baseline|done` and `route|intent` existence, plus permit operation/current boot/PID/birth/config hash, but not an exact ordered D2 phase prefix. Completed-operation maintenance reconciliation is accepted only for the latest completed target B; completed D3 target A refuses. `reconcile_existing` is service-changing because it may create/remove maintenance metadata and stop/start ingress under the journal lock. Neither case provides automatic failover or general recovery.
