...
Ingress startup executes `hat ingress-check`: this is a read-only admission check and does not itself change service state. For an incomplete verified-D3 serving/rejoin-tail state, it requires the maintenance marker, valid complete D3 proof, retained route writer/epoch, and matching current route hash; it does not require a route permit, current boot, or live controller PID/birth. For incomplete `route_pending`, require the exact phase prefix, maintenance marker, no failure, D3 proof as applicable, and a route permit with matching operation/current boot/PID/birth and config hash. `verify_pending` additionally validates the recorded route writer/epoch/config facts. Maintenance remains durable until journal completion. Interrupted operations close ingress and refuse replay. An already-completed operation can reconcile only its exact matching maintenance marker and route hash, without starting a new promotion. `reconcile_existing` is different: it is service-changing because it may create/remove maintenance metadata and stop/start ingress under the journal lock. Neither case provides automatic failover or general recovery.
