...
### Task 9J — Authenticated transport-to-executor boundary — approved for local-only implementation

Design reference: [Task 9J transport executor](2026-09-11-task9j-transport-executor-design.md). This slice connects the accepted local listener to an injected `NodeExecutor` only. No default/native executor, controller wiring, production call site, daemon/service unit, TLS/mTLS, remote forwarding, process, restore, fence, route publication, deployment, or disruptive effect is authorized.
...
### Task 9L — Local authenticated requester — approved for local-only implementation

Design reference: [Task 9L local requester](2026-09-11-task9l-local-requester-design.md). This slice adds one no-retry client helper over the accepted local request/response contract. No production call site, controller wiring, default/native executor, daemon/service unit, TLS/mTLS, remote forwarding, process, restore, fence, route publication, deployment, or disruptive effect is authorized.
