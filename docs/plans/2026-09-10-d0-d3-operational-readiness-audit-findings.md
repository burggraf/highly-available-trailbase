...
- `hat ingress-check` is read-only admission with four branches: completed; incomplete verified-D3 rejoin-tail; incomplete D3 `route_pending`/`verify_pending` with the exact D3 prefix; and incomplete D2 `route_pending`. The D2 branch requires maintenance operation identity, no failure, `baseline|done` and `route|intent` existence, plus permit operation/current boot/PID/birth/config hash, but not an exact ordered D2 phase prefix. Completed-operation maintenance reconciliation is accepted only for latest completed target B; completed D3 target A refuses. `reconcile_existing` is service-changing; `inspect-cold` and `inspect-frozen` may create dispatcher metadata and are not strictly read-only.
...
| `hat ingress-check` | Read-only | Root; no extra arguments. The implemented `hat/control.py::ingress_allowed` branches are distinct: **(1) completed operation** requires maintenance absent and completed route evidence whose config hash equals the current ingress; it does not read/require a route permit, current boot, or PID. **(2) incomplete verified D3 rejoin-tail** requires the maintenance marker plus valid complete D3 proof, retained route writer/epoch, and matching current route hash; it does not require a permit, current boot, or PID. **(3) incomplete D3 `route_pending`/`verify_pending`** requires the exact D3 phase prefix, maintenance/no failure, D3 proof as applicable, and route permit operation/current boot/PID/birth/config hash; `verify_pending` additionally validates recorded route writer/epoch/config facts. **(4) incomplete D2 `route_pending`** requires maintenance operation identity, no failure, `baseline|done` and `route|intent` existence, plus permit operation/current boot/PID/birth/config hash, but not an exact ordered D2 phase prefix. | No intent or retained result; exit 0/1 is the evidence unless the invoking service retains it. Safe to repeat as a gate. | `hat/control.py::{main,ingress_allowed,_d3_serving_state}` completed branch, D3 serving branch, and `route_pending`/`verify_pending` branches; `tests/test_transitions.py::test_completed_crash_reconciles_exact_marker_without_new_operation`; `tests/test_recovery_guards.py::test_d3_gate_recognizes_each_post_verification_rejoin_tail_boundary`; `tests/test_rejoin_driver.py::test_ingress_check_rejects_extra_argument_before_dispatch` | `correct now` |
...
### P0 — completed and incomplete-D2 ingress route admission gaps (`contract missing`)

- **Current evidence:** `ingress_allowed` and `reconcile_existing` bind the current config hash, but the completed branch does not require exact route writer/epoch/key-set identity. The incomplete D2 `route_pending` branch also does not require an exact ordered D2 phase prefix; it checks maintenance operation identity, no failure, `baseline|done` and `route|intent` existence, and the live permit bindings. These gaps are recorded in the route checks above.
