...
1. `surface_manifest.json`: pinned machine-readable census. Every built-in route has exact method, raw path template, handler, source anchor, conditional branch, classification (`allow`, `deny`, or `runtime_unknown`), authoritative effects, and secondary effects. It also lists built-in jobs, feature/config branches, WASM/custom-router registration points, listeners, and direct-writer capability classes. Duplicate entries, missing classifications, unknown fields, source-pin mismatch, or unaccounted entries fail. There is no workload `read` class: every route except the three exact positive controls is denied; oracle reads use a separate fixture capability.
2. `surface_closure.py`: pure validation only. It accepts a bounded raw request plus an independently collected, content-addressed runtime attestation. It has no listener, forwarding client, subprocess call, or mutation. Only the three exact contracts may return an operation/database binding. Everything else refuses before an opener or adapter can run.
...
An allowed request must match all of:
