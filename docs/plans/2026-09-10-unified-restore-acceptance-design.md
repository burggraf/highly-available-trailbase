...
The selected ledger is parsed by the normative checked-in `_protected_ledger` grammar: newline-terminated bounded duplicate-key-rejecting JSONL; first frame has exactly nonempty `auth_token`, `retained_refresh`, and `revoked_refresh`; subsequent frames are exact submitted/acknowledged main/aux rows, exact historical-auth frames, or an exact smoke marker; acknowledged IDs are decimal string/integer identities; submitted/acknowledged row pairs match; and acknowledged coverage includes both main and aux. The oracle replaces `assert`-based `verify_restore` checks with explicit exceptions but preserves these exact semantics. `authentication: PASS` requires every revoked refresh to return 400/401/403, every retained case to match its literal `accepted` (HTTP 200) or `denied` (400/401/403) expectation, and the first accepted refresh to yield a nonempty string access token. `records: PASS` requires that token to read every acknowledged `/api/records/v1/<api>/<id>` with HTTP 200 and exact equality for every ledger `op_key` and `payload`; no token or body is emitted in the result.
