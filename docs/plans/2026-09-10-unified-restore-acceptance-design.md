...
The implementation replaces and strengthens `_protected_ledger` with this normative future grammar. It is UTF-8, newline-terminated, bounded to 4 MiB, has nonempty lines of at most 8192 bytes, and rejects duplicate JSON keys. The first frame has exactly `auth_token`, `retained_refresh`, and `revoked_refresh`, each a nonempty string. There is exactly one final `{"event":"smoke_pass"}` frame. Between them:

- a submitted frame has exactly `event:"submitted"`, `api`, `row`, and positive integer/non-boolean `time_ns`;
- an acknowledged frame has exactly the same plus `event:"acknowledged"` and `id`; `api` is `main_ops|aux_ops`, `row` is exactly `op_key` and `payload`, `op_key` matches `d1-[A-Za-z0-9-]{1,125}`, payload is a string of at most 4096 characters, and ID is a decimal string/integer in `1..2^63-1` with at most 19 digits;
- every unique `(api,op_key)` has one submitted frame followed later by exactly one matching-row acknowledgement, with no unmatched/duplicate outcome, and acknowledgements cover both APIs;
- a historical-auth frame has exactly `event:"historical_auth"`, nonempty string `retained_refresh`, nonempty string `revoked_refresh`, and `retained_expected:"accepted"|"denied"`; zero or more may appear outside a submitted/acknowledged pair.

No other frame, key, order, value type, or multiplicity is accepted. Existing historical artifacts are not rewritten; future new-contract operations require their selected ledger to satisfy this grammar before any oracle action.

The oracle replaces `assert`-based `verify_restore` checks with explicit exceptions. `authentication: PASS` requires every revoked refresh to return 400/401/403, every retained case to match its literal `accepted` (HTTP 200) or `denied` (400/401/403) expectation, and the first accepted refresh to yield a nonempty string access token. `records: PASS` requires that token to read every acknowledged `/api/records/v1/<api>/<id>` with HTTP 200 and a JSON object whose decimal `id` equals the ledger ID and whose `op_key` and `payload` exactly equal the ledger row. Additional response keys are permitted but ignored and are not evidence. No token or response body is emitted in the result.
