...
1. `Journal.step` commits the operation phase intent by successful SQLite COMMIT under `DELETE`/`EXTRA` before any oracle artifact or command. An exact named reconciliation already has that pending phase intent and additionally exclusive-creates/fsyncs its one-shot intent plus directory first.
