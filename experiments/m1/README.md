...
`init-remote` verifies the root-owned, non-symlink `/var/lib/hat-qualification` base, atomically creates a unique `0700` run directory, and removes a newly-created directory if verification fails. Failed local runs remain private for inspection. Task5 retains only registered, top-level databases, ledgers, pinned binaries, copied source, restored data, promoted data, clean data, and process logs that are present; bounded no-follow attestations record each path's category, state, type, owner, and mode, including partial trees. Failed runtime roots and sockets are retained for review, while successful runtime roots remain cleanup candidates. The generated local and remote `task5-remote.py` controls are removed and absence-proven. Local e1/e2 position JSONs and remote raw evidence are intentionally retained. Cleanup PASS is scoped to stopped Task5 processes and removal of remote credentials, configs, processes, and temporary control files; it does not claim whole-run-root removal.
